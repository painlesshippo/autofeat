_autofeat_completion() {
    ...
    *)
    ...
    if [[ "$command" == "new" ]]; then
        if [[ "${COMP_WORDS[COMP_CWORD - 1]}" == "--template" ]]; then
        ...
        elif [[ "${COMP_WORDS[COMP_CWORD - 1]}" == "--ref" ]]; then
            return
        else
            local ref_seen=false
            local template_seen=false
            local source_seen=false
            local index
            ...
            local -a candidates=()
            if [[ "$template_seen" == false && "$source_seen" == false && "$ref_seen" == false ]]; then
                candidates+=(--template)
            fi
            if [[ "$template_seen" == false && "$ref_seen" == false ]]; then
                candidates+=(--ref)
            fi
            local candidate
